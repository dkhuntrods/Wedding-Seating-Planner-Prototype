define(["./PhysicalShape", "./SeatList", "libs/Backbone.Framework"], 

function(PhysicalShape, SeatList) {
    
	var Furniture = PhysicalShape.extend({
		...
		initialize: function (attrs) {
			_.bindAll( this, 'handleDimensionsChange', 'resetSlots', 'handleSeatSlotsChange', 'calculatePositions', 'validateSize');
		...
		setSizeAtScale: function(width, height) {
			var m = this.toJSON(),
			...
			defaultDim = maxDim + m.buffer * 2;			
			newScale = editDim / defaultDim;

			if (newScale != 1 ) this.set({ scaleX: newScale, scaleY: newScale });
		},
		...
		calculatePositions: function (){
			...
			var seatList = [], seatSlots = [];
				...
				this.seats.updateSeats(seatList, seatSlots);
		...
		getSeatsArray : function(seatSlots, totalSeats, type) {
			...
			if (type.id == FurnitureShapeTypes.top.id) {
				tSeats = seatSlots;
		...
		checkClosest: function (x, y) {
			...
			this.seats.each( function (seat) {
				var	x0 = seat.get('x'), y0 = seat.get('y'),
